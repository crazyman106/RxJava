...
// MediaRecorder， MediaProjection 实现手机录屏功能
/**
 * MediaProjection 是Android5.0后才开放的屏幕采集Api,通过系统级服务MediaProjectionManager进行管理
 *
 * 录制可以分为两个部分:即通过MediaProjectionManager申请录屏权限,用户允许后开始录制屏幕,
 * 然后通过MediaRecord对音频数据进行采集处理
 *
